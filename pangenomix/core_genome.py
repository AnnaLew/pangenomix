...
def create_core_genes_fasta(allele_npz_file, allele_npz_label_file, gene_npz_file, gene_npz_label_file, input_faa, genomes_num, output_faa):
    ...
    highest_expression_rows = find_highest_allele_expression(allele_occurence_count, highest_allele_expression_rows)
...
def find_highest_allele_expression(row_counts_df, gene_allele_rows_df):
    ...
    for index, row in gene_allele_rows_df.iterrows():
        ...
        # Append the result to the output data list only if highest_rows is greater than 25
        if is_tie:
...
def count_gene_occurence(gene_npz_file):
    ...
    df['count'] = df.groupby('gene_index').transform('size')
